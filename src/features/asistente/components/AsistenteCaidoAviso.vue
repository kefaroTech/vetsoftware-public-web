 ...
 *
 * ── Por qué la salida de los paquetes SÍ se conserva ────────────────────────
 * Porque esa sí existe. «O empieza por un paquete ya armado» se sirve de
 * `PLANS_CONTENT` —contenido local del front, ver `landing/api/plans.source.ts`,
 * que hoy no toma ningún endpoint— y no de `GET /catalog`, así que sigue llena
 * con el catálogo comercial vacío. Retirarla «por simetría» sería el error del
 * revés: dejar al visitante sin la única compra que en ese momento puede hacer.
 *
 ...
 * la etiqueta y su prop, y el porqué vive aquí.
 *
