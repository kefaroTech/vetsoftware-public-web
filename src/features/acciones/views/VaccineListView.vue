...
onMounted(async () => {
  loading.value = true
  try {
    items.value = await vaccinationApi.listAll()
  } catch (e) {
...
<template>
  <div class="page">
    ...
    <ListBody
      ...
      :loading="loading"
      :search-fn="searchFn"
      placeholder="Buscar paciente, vacuna o lote…"
      empty-text="Aún no hay vacunaciones registradas."
    >
      <template #header>
        <tr>
          <th>Fecha</th>
    ...
    <VaccineCreateModal :open="modalOpen" @close="modalOpen = false" @created="onCreated" />
  </div>
</template>
