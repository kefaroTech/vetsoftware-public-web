...
onMounted(async () => {
  ...
  error.value = null
  try {
    items.value = await laboratoryTestApi.listAll()
  } catch (e) {
    error.value =
  ...
  <div class="page">
    <div class="header">
      <div>
        ...
        <h1 class="title">Solicitudes de laboratorio</h1>
        <div class="lead">Crea y consulta solicitudes de examen sin pasar por una consulta.</div>
      </div>
    ...
    <ListBody
      ...
      :loading="loading"
      :search-fn="searchFn"
      placeholder="Buscar paciente, tipo o diagnóstico…"
      empty-text="Aún no hay solicitudes de laboratorio."
    >
      <template #header>
        <tr>
          <th>Fecha</th>
    ...
    <LabCreateModal :open="modalOpen" @close="modalOpen = false" @created="onCreated" />
  </div>
</template>
...
.kicker {
  font-size: 12px; color: var(--warm-500);
  letter-spacing: 0.06em; text-transform: uppercase; margin-bottom: 6px;
}
.title {
  margin: 0; font-family: var(--font-serif); font-size: 38px;
  line-height: 1.05; font-weight: 400; letter-spacing: -0.015em; color: var(--warm-900);
}
