 ...
 */
async function requestDelete(target: LaboratoryTestResponse) {
  error.value = null
  try {
    const ok = await confirm({
      title: 'Eliminar examen',
      message: `Se eliminará la solicitud de ${target.testType.name}.`,
      consequence: 'Esta acción no se puede deshacer.',
      confirmLabel: 'Eliminar',
      busyLabel: 'Eliminando…',
      action: () => laboratoryTestApi.remove(target.id),
    })
    if (!ok) return
    void listBody.value?.reload()
