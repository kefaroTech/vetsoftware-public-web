...
<template>
  <div class="pagination" v-if="pageCount > 1 || total > 0">
    <div class="summary">
      Mostrando {{ range.from }}–{{ range.to }} de {{ total }}
    </div>
    <nav class="nav" aria-label="Paginación">
      <button type="button" class="ctrl" :disabled="page === 1" @click="go(1)" aria-label="Primera">
        <ChevronsLeft :size="14" :stroke-width="1.7" />
      </button>
      <button type="button" class="ctrl" :disabled="page === 1" @click="go(page - 1)" aria-label="Anterior">
