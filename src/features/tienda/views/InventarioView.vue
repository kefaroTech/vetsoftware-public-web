...
<template>
  <div class="inv">
    ...
    <div
      v-if="mode === 'active' && showStock && lowCount > 0"
      class="alert"
      role="button"
      tabindex="0"
      @click="showLowStock"
      @keyup.enter="showLowStock"
    >
    ...
    <template v-if="mode === 'active'">
      <div class="filters">
        ...
        <select v-model="cat" class="fsel">
          ...
          <option v-for="c in store.productCategories.value" :key="c.id" :value="String(c.id)">
            {{ c.name }}
          </option>
        </select>
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr v-for="p in slice" v-else :key="p.id" class="trow" @click="onRowClick(p)">
            ...
            <td>
              <StockStatePill v-if="showStock" :state="stateOf(p)" /><span v-else class="muted"
                >—</span
              >
            </td>
            <td class="tactions" @click.stop>
              ...
              <button
                v-if="canTransfer && showStock && transferBranchOptions.length > 0"
                type="button"
                class="iconbtn"
                title="Transferir"
                @click="transferFor = p"
    ...
    <AdjustModal
      ...
      :product="adjustFor"
      :branch-name="branchName"
      :current="adjustFor ? stockOf(adjustFor).quantity : undefined"
    ...
    <TransferModal
      ...
      :current="transferFor ? stockOf(transferFor).quantity : undefined"
    ...
    <ConsumeModal
      ...
      :product="consumeFor"
      :branch-name="branchName"
      :current="consumeFor ? stockOf(consumeFor).quantity : undefined"
