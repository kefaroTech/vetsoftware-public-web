...
<template>
  <ModalShell
    ...
    :subtitle="product ? `${product.name}${branchName ? ` · ${branchName}` : ''}${current != null ? ` · disponible ${current} u` : ''}` : ''"
    :icon="Syringe"
    :width="460"
    @close="emit('close')"
  >
    <template #body>
      <div class="body">
        ...
        <BaseField label="Motivo / paciente">
          <template #default="{ id }">
            <BaseTextarea :id="id" v-model="form.reason" :rows="2" placeholder="Opcional (paciente, procedimiento…)" />
          </template>
        </BaseField>
      </div>
    </template>
    ...
    <template #footer-actions>
      ...
      <button type="button" class="btn-primary" @click="submit">Registrar consumo</button>
    </template>
  </ModalShell>
</template>
