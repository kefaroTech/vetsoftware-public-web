...
const destError = computed(() => (submitted.value && !form.toBranchId ? 'Selecciona la sede destino' : undefined))
...
function submit() {
  ...
  if (!form.toBranchId || qtyN.value <= 0 || (props.current != null && qtyN.value > props.current)) return
  emit('confirm', { toBranchId: Number(form.toBranchId), quantity: qtyN.value, reason: form.reason.trim() || null })
}
</script>
...
<template>
  <ModalShell
    ...
    :subtitle="product ? `${product.name}${fromBranchName ? ` · desde ${fromBranchName}` : ''}${current != null ? ` · disponible ${current} u` : ''}` : ''"
    :icon="ArrowLeftRight"
    :width="480"
    @close="emit('close')"
  >
    <template #body>
      <div class="body">
        <BaseField label="Sede destino" required :error="destError">
          <template #default="{ id }">
            <BaseSelect :id="id" v-model="form.toBranchId" :options="branchOptions" :invalid="!!destError" placeholder="Selecciona…" />
          </template>
        </BaseField>
