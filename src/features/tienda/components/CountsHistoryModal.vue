...
async function load(p = 0) {
  ...
  error.value = null
  try {
    const res = await inventoryApi.counts({ branchId: props.branchId, page: p, pageSize: PAGE_SIZE })
  ...
  >
    <template #body>
      ...
      <template v-if="detail">
        <button type="button" class="back" @click="detail = null"><ArrowLeft :size="14" :stroke-width="1.8" /> Volver al historial</button>
        <div class="meta">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-if="detailLoading"><td colspan="4" class="empty">Cargando…</td></tr>
            <tr v-for="l in detail.lines" v-else :key="l.productId">
              ...
              <td class="num">
                <span class="diff" :class="{ zero: l.difference === 0, neg: l.difference < 0, pos: l.difference > 0 }">
      ...
      <template v-else>
        <table class="table">
          ...
          <tbody>
            ...
            <tr v-else-if="rows.length === 0"><td colspan="5" class="empty">Aún no hay conteos registrados.</td></tr>
            <tr v-for="r in rows" v-else :key="r.id" class="trow" @click="openDetail(r.id)">
              ...
              <td class="num"><span :class="{ badge: r.adjustedLines > 0 }">{{ r.adjustedLines }}</span></td>
...
.back { display: inline-flex; align-items: center; gap: 6px; background: transparent; border: none; color: var(--amatista-700); font-family: inherit; font-size: 12.5px; font-weight: 500; cursor: pointer; padding: 0; margin-bottom: 12px; }
.back:hover { text-decoration: underline; }
.meta { display: flex; flex-wrap: wrap; gap: 14px; margin-bottom: 12px; font-size: 12.5px; color: var(--warm-600); }
.meta .note { color: var(--warm-500); font-style: italic; }
...
.tnote { color: var(--warm-600); }
.sys { color: var(--warm-600); }
.badge { display: inline-flex; min-width: 20px; justify-content: center; padding: 1px 7px; border-radius: 999px; background: var(--amatista-50); color: var(--amatista-700); font-weight: 600; font-size: 11.5px; }
.diff { font-weight: 600; }
.diff.zero { color: oklch(55% 0.12 150); }
.diff.neg { color: oklch(52% 0.18 25); }
.diff.pos { color: oklch(52% 0.12 70); }
.chev { color: var(--warm-400); }
