...
function selectTab(nextTab: CartKind) {
  tab.value = nextTab
  query.value = ''
}
...
async function confirm() {
  ...
  try {
    ...
    if (destino.value === 'existing') {
      ...
      toast.success(
        'Cuenta abierta',
        `Se creó una cuenta para ${props.ownerName} con ${units} cargo(s).`,
      )
    }
    emit('finish')
    emit('close')
...
<template>
  ...
  >
    <template #body>
      ...
      <template v-else>
        ...
        <div class="dest">
          ...
          >
            <span class="do-check"
              ><Check v-if="destino === 'existing'" :size="13" :stroke-width="3"
            /></span>
            <span class="do-text">
          ...
          <button
            ...
            type="button"
            class="destopt"
            :class="{ active: destino === 'new' }"
            @click="destino = 'new'"
          >
            <span class="do-check"
              ><Check v-if="destino === 'new'" :size="13" :stroke-width="3"
            /></span>
            <span class="do-text">
          ...
          <button
            type="button"
            class="destopt"
            :class="{ active: destino === 'nada' }"
            @click="destino = 'nada'"
          >
            <span class="do-check"
              ><Check v-if="destino === 'nada'" :size="13" :stroke-width="3"
            /></span>
            <span class="do-text">
            ...
            <ul class="catlist">
              ...
              >
                ...
                <button
                  ...
                  @click="addItem(tab, it.id, it.name, it.price, it.soldOut)"
                >
          ...
          <div class="selected">
            ...
            <ul v-if="items.length" class="sel-list">
              <li v-for="line in items" :key="`${line.kind}-${line.id}`" class="selrow">
                ...
                <span class="stepper">
                  <button type="button" class="st-btn" @click="setQty(line, line.qty - 1)">
                ...
                <button type="button" class="sel-remove" title="Quitar" @click="removeLine(line)">
    ...
    <template #footer-actions>
      <button v-if="dismissible" type="button" class="btn-ghost" @click="emit('close')">
        Cancelar
      </button>
