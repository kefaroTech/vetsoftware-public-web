...
/**
 * Store de Cuentas abiertas (por propietario), module-scoped. El total/saldo
 * proviene del backend; los cargos se agrupan por mascota en la UI.
 */
...
async function loadDetail(accountId: number): Promise<void> {
  ...
  try {
    ...
    for (const c of prod.filter((x) => x.enabled)) {
      const price =
        c.product.salePrice ??
        tienda.products.value.find((p) => p.id === c.product.id)?.salePrice ??
        0
      unified.push({
    ...
    for (const c of svc.filter((x) => x.enabled)) {
      const price =
        c.service.price ??
        tienda.services.value.find((s) => s.id === c.service.id)?.price ??
        0
      unified.push({
...
export function useCuentas() {
  ...
  /** Busca una cuenta abierta del propietario (regla: máx 1 por dueño). */
  async function findOpenAccountByOwner(ownerId: number): Promise<OpenAccountResponse | null> {
    const local = accounts.value.find((a) => a.owner.id === ownerId && a.enabled)
    if (local) return local
    const res = await openAccountApi.search({ ownerId, enabled: true, page: 0, pageSize: 1 })
  ...
  /** Recarga la cuenta (para refrescar totales backend) y su detalle. */
