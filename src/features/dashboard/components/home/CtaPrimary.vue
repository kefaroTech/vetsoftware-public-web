...
<template>
  <section class="cta-primary">
    ...
    <div class="content">
      ...
      <p class="desc">
        Registra el motivo, examen físico, diagnóstico y tratamiento del paciente en
        una sola pantalla.
      </p>
      <div class="actions">
        ...
        <button type="button" class="btn-ghost" @click="goHistorial">
          Ver historial
        </button>
      </div>
    </div>
...
.cta-primary {
  ...
  box-shadow:
    ...
    0 8px 24px -8px oklch(40% 0.18 var(--hue) / 0.5);
  transition: transform 0.15s ease, box-shadow 0.15s ease;
}
.cta-primary:hover {
  transform: translateY(-1px);
  box-shadow:
    0 1px 2px rgba(50, 20, 80, 0.1),
    0 12px 28px -8px oklch(40% 0.18 var(--hue) / 0.55);
}
.decor {
  position: absolute;
  top: -60px;
...
.btn-ghost {
  background: oklch(60% 0.10 var(--hue) / 0.25);
  color: white;
  border-color: oklch(80% 0.06 var(--hue) / 0.3);
}
.btn-ghost:hover {
  background: oklch(60% 0.10 var(--hue) / 0.35);
}
</style>
