    ...
    <svg viewBox="-40 -40 200 200" aria-hidden="true">
      <defs>
        <filter v-if="props.glow" :id="filterId" x="-60%" y="-60%" width="220%" height="220%">
          ...
          <feFlood flood-color="var(--amatista-600)" flood-opacity="0.55" result="color1" />
          <feComposite in="color1" in2="blur1" operator="in" result="glow1" />
          <feGaussianBlur in="SourceAlpha" stdDeviation="9" result="blur2" />
          <!-- `--amatista-500` (58 %) y no `--amatista-600` (51,1 %) a propósito:
               el halo exterior tiene que verse más claro que el interior. -->
          <feFlood flood-color="var(--amatista-500)" flood-opacity="0.45" result="color2" />
