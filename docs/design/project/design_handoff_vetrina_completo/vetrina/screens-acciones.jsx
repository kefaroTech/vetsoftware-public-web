...
function VetPatientCascadePicker({ onSelect }) {
  ...
  const [pSpecie, setPSpecie] = React.useState('Canino');
  ...
  function savePet() {
    ...
    const a = { id: 'newp-' + Date.now().toString(36), name: pName.trim(), specie: { name: pSpecie }, breed: { name: '—' }, _new: true };
    ...
    <div className="vet-picker">
      {!selectedOwner ? (
        ...
        <div className="vet-picker-step">
          ...
          {petForm ? (
            <div className="vet-picker-form">
              <div className="vet-picker-form-grid two">
                <input className="vet-picker-finput" value={pName} onChange={(e) => setPName(e.target.value)} placeholder="Nombre de la mascota *" autoFocus />
                <select className="vet-picker-finput" value={pSpecie} onChange={(e) => setPSpecie(e.target.value)}>
                  {['Canino', 'Felino', 'Ave', 'Conejo', 'Otro'].map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <div className="vet-picker-form-actions">
                <button type="button" className="vet-btn-ghost-modal" onClick={() => setPetForm(false)}>Cancelar</button>
                <button type="button" className="vet-btn-primary-modal" disabled={!pName.trim()}
                  style={!pName.trim() ? { opacity: 0.5, cursor: 'not-allowed' } : null}
                  onClick={() => { const a = { id: 'newp-' + Date.now().toString(36), name: pName.trim(), specie: { name: pSpecie }, breed: { name: '—' }, _ownerId: selectedOwner.id, _new: true }; setExtraPets((arr) => [...arr, a]); setPetForm(false); pickAnimal(a); }}>
            ...
            <>
              {animals.length === 0 ? (
                <div className="vet-picker-empty">
                  ...
                  <button type="button" className="vet-cpk-newbtn" onClick={() => { setPName(''); setPetForm(true); }}>
                ...
                <>
                  ...
                  <button type="button" className="vet-cpk-newbtn ghost" onClick={() => { setPName(''); setPetForm(true); }}>
                    <VetIcons.Plus size={15} strokeWidth={2.2} /> Registrar mascota nueva
                  </button>
                </>
              )}
