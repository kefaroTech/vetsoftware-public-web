...
3. **Documentos electrónicos** — listado + detalle (§4)
   - Cobro/cierre de venta → **auto-emite** el documento (§4.1)
...
### 4.0 De dónde sale una factura — AUTO-disparo al cobrar/cerrar
Un documento se **emite a partir de una venta** = una `OpenAccount` (cuenta de cobro) **cerrada**
(estado `CLOSE`, saldo 0). **La emisión es AUTOMÁTICA al cerrar/cobrar la venta**: cuando la cuenta pasa a
`CLOSE` (cobro en POS o cierre de una cuenta de crédito), el backend dispara la emisión del documento
electrónico **por sí solo**. El usuario no llama a un endpoint de emisión aparte.

Comportamiento del auto-disparo (importante para el diseño):
- **Best-effort, no bloquea la venta.** La emisión corre **después** de cerrar la cuenta y en segundo plano:
  si la DIAN está caída o falta configuración (numeración/proveedor), **la venta se cierra igual**; el
  documento queda pendiente/contingencia para reintento. El cierre **nunca** falla por la facturación.
- **Asíncrono.** Tras el cierre, el documento nace `PENDIENTE` y la validación DIAN llega luego (webhook/
  polling). El front no debe esperar el CUFE en la respuesta del cierre.
- **Idempotente.** Una cuenta genera **un** documento; reintentar el cierre no duplica.
- **Premium vs no premium.** Empresa premium (con `BILLING`) → se transmite a la DIAN. Sin premium → el
  documento se guarda **local** `NO_ELECTRONICO` (nunca va a la DIAN). En ambos casos la venta se cierra.

### 4.1 Cerrar/cobrar la venta (acción principal que dispara la emisión)
- **Endpoint:** `PATCH /open-accounts/{id}/status` (feature cuentas) → cierra la cuenta y auto-dispara la emisión.
- **Request `ChangeOpenAccountStatusRequest`** (campos relevantes a la emisión):

| Campo | Tipo | UI | Lo envía el cliente |
|---|---|---|---|
| `status` | String | `"CLOSE"` para cerrar/cobrar (también `"CANCEL"` para anular) | Sí |
| `documentType` | String | Solo en CLOSE: `"DOC_EQUIV_POS"` (venta de mostrador) o `"FE_VENTA"` (factura); si se omite → `DOC_EQUIV_POS` | Sí |
| `finalConsumer` | boolean | Solo en CLOSE: toggle "Consumidor final" (NIT genérico 222222222222) | Sí |
| `reason` | String | Requerido solo en `CANCEL` | Condicional |

- **UI del cobro/cierre:**
  - Pre-requisito: registrar el/los pagos (abonos) hasta dejar **saldo 0** (el backend exige saldo 0 para CLOSE).
  - Selector tipo de documento (**POS** por defecto / **Factura**), toggle "Consumidor final".
  - Si "Consumidor final" = off y tipo = Factura → validar que el cliente tenga datos fiscales completos
    (§5.1); si faltan, ofrecer "Completar datos del cliente" (sin bloquear el cobro si el negocio lo permite).
  - Botón **Cobrar / Cerrar venta**. Tras el cierre, mostrar feedback "venta cerrada · factura en proceso"
    y reflejar el documento como `PENDIENTE` (validando…). No prometer CUFE inmediato.
- **Emisión manual (alternativa/edge, normalmente no en el flujo POS):** sigue existiendo
  `POST /electronic-documents/emit` (request `BuildElectronicDocumentRequest`: `openAccountId`,
  `documentType`, `finalConsumer`) para re-emitir o casos especiales con permiso `electronicDocument.emit`.
  El provisional `POST /electronic-documents/from-account` construye sin transmitir (previsualización).
...
3. **Emisión automática y asíncrona al cobrar/cerrar:** la venta dispara la emisión sola (§4.0); es
   *best-effort* (no bloquea ni revierte la venta) y no da CUFE al instante. Diseñar el cierre para no
   esperar la DIAN: "venta cerrada · factura en proceso", estado `PENDIENTE` + refresco; nunca prometer
   "validado" antes de tiempo.
...
| **Cerrar/cobrar venta → AUTO-emite** | PATCH | `/open-accounts/{id}/status` (body: `status=CLOSE`, `documentType`, `finalConsumer`) | `openAccount.update` |
| Emitir (manual / re-emisión) | POST | `/electronic-documents/emit` | `electronicDocument.emit` |
