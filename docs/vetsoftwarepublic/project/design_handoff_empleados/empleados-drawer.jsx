      ...
      <aside style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            <Avatar initials={employee.initials} size={64} active={employee.active} roleColor={role.color} />
            <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                <RolePill roleId={employee.role} size="lg" />
        ...
        <div style={{
          ...
          <button style={tabBtn('rol', 'Rol & permisos')} onClick={() => setTab('rol')}>Rol & permisos</button>
        </div>
          ...
          {tab === 'rol' && (
            <TabRol
              ...
              onChangeRole={(newRoleId) => {
                onUpdate(employee.id, { role: newRoleId });
...
// ─── Tab: Rol & permisos ───
function TabRol({ employee, roleEditOpen, setRoleEditOpen, onChangeRole }) {
  const role = ROLES.find(r => r.id === employee.role);
  const c = ROLE_COLORS[role.color];
    ...
    return (
      <div>
        <div style={{ fontSize: 13, color: 'var(--warm-600)', marginBottom: 14 }}>
          Selecciona el nuevo rol para <strong style={{ color: 'var(--warm-900)' }}>{employee.fullName.split(' ')[0]}</strong>:
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {ROLES.map(r => {
            const rc = ROLE_COLORS[r.color];
            const selected = r.id === employee.role;
            return (
              <button key={r.id}
                ...
                style={{
                  ...
                  padding: '12px 14px',
                  background: selected ? rc.bg : 'var(--warm-50)',
                  border: selected ? `1.5px solid ${rc.dot}` : '1px solid var(--warm-200)',
              ...
              >
                <div style={{
                  ...
                  background: rc.dot, marginTop: 7, flexShrink: 0,
                }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--warm-900)' }}>{r.name}</span>
                    {selected && <span style={{ fontSize: 11, color: rc.fg, fontWeight: 500 }}>· actual</span>}
                  </div>
                  <div style={{ fontSize: 12.5, color: 'var(--warm-600)', marginTop: 4, lineHeight: 1.45 }}>
                    {r.description}
                  </div>
                </div>
                {selected && <IconCheck size={16} style={{ color: rc.fg, marginTop: 2 }} />}
              </button>
            );
          })}
        </div>
        <button onClick={() => setRoleEditOpen(false)} style={{
          marginTop: 14, padding: '8px 14px', fontSize: 13,
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              background: 'var(--warm-50)', color: c.fg,
              display: 'grid', placeItems: 'center',
            }}>
              <IconShield size={17} />
            </div>
            <div>
              <div style={{ fontSize: 11.5, color: c.fg, opacity: 0.8, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Rol actual</div>
              <div style={{ fontSize: 16, fontWeight: 600, color: c.fg, marginTop: 2 }}>{role.name}</div>
            </div>
          </div>
          <button onClick={() => setRoleEditOpen(true)} style={{
            padding: '7px 12px', fontSize: 12.5, fontWeight: 500,
            background: 'var(--warm-50)', color: c.fg,
            border: `1px solid ${c.fg}`, borderRadius: 7,
            cursor: 'pointer', fontFamily: 'inherit',
        ...
        <div style={{ fontSize: 13, color: c.fg, opacity: 0.85, marginTop: 12, lineHeight: 1.5 }}>
          {role.description}
        </div>
      </div>
    </div>
