...
function vetWeightHistory(pet) {
  // Serie de peso derivada del peso actual (mock, más reciente primero)
  const base = Number(pet.weight) || 5;
  const months = ['may 2026', 'feb 2026', 'nov 2025', 'ago 2025', 'may 2025'];
  return months.map((date, i) => ({ date, kg: +(base - i * (base * 0.03)).toFixed(1) }));
}
  ...
  return (
    <VetHistoriaShell>
      <div className="vet-step" style={{ maxWidth: '100%', padding: 0 }}>
        ...
        <div className="vet-weight-section">
          <button
            type="button"
            className={'vet-weight-toggle' + (showWeight ? ' open' : '')}
            aria-expanded={showWeight}
            onClick={() => setShowWeight((v) => !v)}
          >
            <VetIcons.BarChart3 size={15} strokeWidth={1.8} />
            <span>{showWeight ? 'Ocultar historial de peso' : 'Ver historial de peso'}</span>
            <VetIcons.ChevronDown size={15} strokeWidth={1.8} className="vet-weight-chev" />
          </button>
          {showWeight && pet && (
            <div className="vet-weight-panel">
              <div className="vet-weight-rows">
                {vetWeightHistory(pet).map((w, i, arr) => {
                  const prev = arr[i + 1];
                  const delta = prev ? +(w.kg - prev.kg).toFixed(1) : 0;
                  return (
                    <div key={i} className="vet-weight-row">
                      <span className="vet-weight-date">{w.date}</span>
                      <span className="vet-weight-kg">{w.kg} kg</span>
                      {prev ? (
                        <span className={'vet-weight-delta ' + (delta > 0 ? 'up' : delta < 0 ? 'down' : 'flat')}>
                          {delta > 0 ? '▲' : delta < 0 ? '▼' : '—'} {Math.abs(delta)} kg
                        </span>
                      ) : <span className="vet-weight-delta flat">registro inicial</span>}
                    </div>
                  );
                })}
              </div>
