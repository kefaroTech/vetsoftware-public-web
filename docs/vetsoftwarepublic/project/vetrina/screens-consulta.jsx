...
function VetSaveConsultaConfirmDialog({ open, petName, saving, onCancel, onConfirm }) {
  React.useEffect(() => {
    if (!open) return;
  ...
  if (!open) return null;
  return (
    <div className="vet-resume-overlay" role="alertdialog" aria-modal="true">
      <div className="vet-resume-card" style={{ width: 440 }}>
        <div className="vet-resume-ic"><VetIcons.Stethoscope size={22} strokeWidth={1.8} /></div>
        <h2 className="vet-resume-title" style={{ fontFamily: 'var(--font-sans)', fontSize: 18, fontWeight: 500 }}>¿Guardar la consulta?</h2>
        <p className="vet-resume-desc">
          Se registrará la consulta{petName && <> de <strong>{petName}</strong></>} y los procedimientos que hayas agregado. A continuación podrás facturarla.
        </p>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          <button type="button" className="vet-resume-ghost" disabled={saving} onClick={onCancel}>Seguir editando</button>
          <button type="button" className="vet-resume-primary" disabled={saving} onClick={onConfirm}>
...
function VetNuevaView() {
  ...
  return (
    <div className="vet-wizard">
      ...
      <VetSaveConsultaConfirmDialog
        open={saveConfirmOpen}
        petName={draft.state.pet?.name}
        onCancel={() => setSaveConfirmOpen(false)}
        onConfirm={() => { setSaveConfirmOpen(false); setBillingOpen(true); }}
      />
...
Object.assign(window, {
  ...
  VetConsultaActiveBanner, VetResumeOrNewDialog, VetDiscardDialog, VetSaveConsultaConfirmDialog,
});
