...
| `registro`     | rojo   | La navegación ya está arreglada: `page.goto('/registro')`, y está **verificado en ejecución** — la pantalla carga y el `h1` «Crear cuenta» resuelve. Sigue rojo por debajo de eso: las etiquetas son «Número de documento (obligatorio)», no «Número de documento \*»; los desplegables son `<select>` nativos y no Vuetify (`.v-input`, `.v-list-item` y `.v-alert` no existen en el formulario); el reCAPTCHA es obligatorio y el spec no lo marca; y el alta ya no auto-loguea (verificación por correo), así que `waitForURL(/dashboard/)` no puede cumplirse nunca. El `beforeAll` agota sus 240 s en el primer `fill` y arrastra los 10 casos. |
